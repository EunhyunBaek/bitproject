...
// -----------------------------------------------------------------------------
// [DB 초기화 및 데이터 보정] 테이블 생성, 기존 ::1 IP 변환 및 초기 테스트 계정 데이터 삽입
// -----------------------------------------------------------------------------
db.serialize(() => {
  ...
  // 3. 기존 DB에 ::1 형태로 저장된 감사 로그 IP 일괄 보정 (::1 -> 127.0.0.1)
  db.run(`UPDATE audit_logs SET ip_address = '127.0.0.1' WHERE ip_address = '::1' OR ip_address = '::ffff:127.0.0.1'`);
...
// -----------------------------------------------------------------------------
// 프록시 환경의 실제 접속 사용자 IP 추출 및 IPv6/루프백 강력 정제 유틸리티
const getClientIp = (req) => {
  if (!req) return '127.0.0.1';

  // 1. 헤더 우선 순위별 IP 확인 (Cloudflare, Nginx, Render 프록시 대응)
  const cfIp = req.headers['cf-connecting-ip'];
  const realIp = req.headers['x-real-ip'];
  const forwarded = req.headers['x-forwarded-for'];

  let ip = cfIp || realIp || (forwarded ? forwarded.split(',')[0].trim() : req.ip);

  // 2. req.socket / connection fallback
  if (!ip || ip === '::1') {
    ip = req.socket?.remoteAddress || req.connection?.remoteAddress || '127.0.0.1';
  }

  // 3. IPv6 루프백 및 IPv4 매핑 형태 제거 (::1, ::ffff:127.0.0.1 -> 127.0.0.1)
...
// 보안 작업 수행 시 DB에 로그를 기록하는 함수 (req 객체를 직접 받거나 IP 문자열을 수용)
const logAudit = (actorId, actorUsername, action, targetEmployeeId, reqOrIp) => {
  let ipAddress = '127.0.0.1';

  if (typeof reqOrIp === 'string') {
    ipAddress = reqOrIp;
  } else if (reqOrIp && typeof reqOrIp === 'object') {
    ipAddress = getClientIp(reqOrIp);
  }
  ...
  db.run(
    ...
    [actorId, actorUsername, action, targetEmployeeId || 'N/A', ipAddress]
  );
};
  ...
  jwt.verify(token, JWT_SECRET, (err, user) => {
    ...
    db.get('SELECT * FROM users WHERE id = ?', [user.id], (err, dbUser) => {
      ...
      if (dbUser.is_retired === 1) {
        logAudit(dbUser.id, dbUser.username, 'BLOCKED_RETIRED_ACCESS_ATTEMPT', dbUser.employeeId, req);
...
const isAdmin = (req, res, next) => {
  if (req.user.role !== 'ADMIN') {
    logAudit(req.user.id, req.user.username, 'UNAUTHORIZED_ADMIN_ACCESS_ATTEMPT', 'N/A', req);
...
app.post('/api/login', loginLimiter, (req, res) => {
  ...
  db.get('SELECT * FROM users WHERE username = ?', [username], async (err, user) => {
    ...
    if (!validPassword) {
      logAudit(user.id, user.username, 'LOGIN_FAILED_BAD_PASSWORD', user.employeeId, req);
      return res.status(400).json({ message: '인증 정보가 올바르지 않습니다.' });
    }

    // JWT 토큰 발급 (유효기간: 2시간)
    const token = jwt.sign({ id: user.id, role: user.role }, JWT_SECRET, { expiresIn: '2h' });
    logAudit(user.id, user.username, 'LOGIN_SUCCESS', user.employeeId, req);
...
app.put('/api/change-password', authenticateToken, async (req, res) => {
  ...
  db.run('UPDATE users SET password = ? WHERE id = ?', [hash, req.user.id], function(err) {
    ...
    logAudit(req.user.id, req.user.username, 'CHANGE_PASSWORD_SUCCESS', req.user.employeeId, req);
...
app.put('/api/profile', authenticateToken, (req, res) => {
  ...
  db.run('UPDATE users SET name = ?, birthDate = ? WHERE id = ?', [name, birthDate, req.user.id], function(err) {
    ...
    logAudit(req.user.id, req.user.username, 'PROFILE_UPDATE', req.user.employeeId, req);
...
app.post('/api/admin/employees', authenticateToken, isAdmin, async (req, res) => {
  ...
  db.run(
    ...
    function(err) {
      ...
      logAudit(req.user.id, req.user.username, 'CREATE_EMPLOYEE', employeeId, req);
...
app.patch('/api/admin/employees/:id/retire', authenticateToken, isAdmin, (req, res) => {
  db.run('UPDATE users SET is_retired = 1 WHERE id = ?', [req.params.id], function(err) {
    ...
    logAudit(req.user.id, req.user.username, 'RETIRE_EMPLOYEE', `USER_ID:${req.params.id}`, req);
...
app.post('/api/admin/background-checks', authenticateToken, isAdmin, async (req, res) => {
  ...
  try {
    ...
    logAudit(req.user.id, req.user.username, 'REQUEST_BACKGROUND_CHECK', employeeId, req);
...
app.get('/api/admin/background-checks/:checkId', authenticateToken, isAdmin, async (req, res) => {
  try {
    ...
    logAudit(req.user.id, req.user.username, 'VIEW_BACKGROUND_CHECK_DETAIL', req.params.checkId, req);
...
app.get('/api/admin/background-checks', authenticateToken, isAdmin, async (req, res) => {
  ...
  try {
    ...
    logAudit(req.user.id, req.user.username, 'VIEW_BACKGROUND_CHECK_HISTORY', employeeId, req);
    res.json(response.data);
  } catch (error) {
    res.status(error.response?.status || 500).json({ message: '조회 실패' });
  }
